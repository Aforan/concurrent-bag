...
import bag.ConcurrentMemoryManagedBag;
...
public class PerformanceTest {
    ...
    private static class ConcurrentMemoryManagedBagTestThread extends Thread {
        private static AtomicInteger addCount = new AtomicInteger(0);
        private static AtomicInteger registrationCount = new AtomicInteger(0);

        private static ConcurrentMemoryManagedBag<Integer> bag = new ConcurrentMemoryManagedBag<>();
        private int threadindex, nthreads, totaloperations;
        private double addratio;

        public ConcurrentMemoryManagedBagTestThread(int threadindex, int nthreads, int totaloperations, double addratio) {
            this.threadindex = threadindex;
            this.nthreads = nthreads;
            this.totaloperations = totaloperations;
            this.addratio = addratio;
        ...
        public void run() {
            ...
            for(int i=0; i<1024; i++) {
                try {
                    bag.add(i);
                } catch (ConcurrentMemoryManagedBag.NotRegisteredException e) {
                    e.printStackTrace();
                }
            }
            ...
            for(int i=1024; i<operations; i++) {
                ...
                try {
                    ...
                    e.printStackTrace();
                    return;
                } catch (ConcurrentMemoryManagedBag.CannotStealException e) {
                    logger.debug("Thread " + threadindex + " Cannot Steal Exception");
                    e.printStackTrace();
                }
            }
    ...
    public static void main(String[] args) {
        ...
        parser.addArgument("-d").help("Data structure [bag|queue|transfer|managed]");
        ...
        try {
            ...
            if(dataStructure.equals("bag")) {
                ...
                logger.debug("Overall Execution Time: " + executionTime + "ms");
                logger.debug("True Execution Time: " + trueExecutionTime + "ms");
            } else if(dataStructure.equals("managed")) {
                LinkedList<ConcurrentMemoryManagedBagTestThread> testThreads = new LinkedList<>();

                for(int i=0;i<nthreads;i++) {
                    testThreads.add(new ConcurrentMemoryManagedBagTestThread(i, nthreads, nOperations, addRatio));
                }
